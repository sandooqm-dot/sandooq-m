// functions/api2/settings-transfer.js
// v1 - Horof settings transfer: save/read one temporary transfer snapshot by (game + email + activation_code)

const VERSION = "settings-transfer-v1";
...
async function getExistingTransfer(db, game, email, activationCode) {
  return await db.prepare(`
    SELECT game, email, activation_code, snapshot_json, source_device_id, created_at, updated_at
...
async function saveTransfer(db, game, email, activationCode, snapshot) {
  const nowIso = new Date().toISOString();
  const sourceDeviceId = limitText(snapshot?.sourceDeviceId, 120);
  const snapshotJson = JSON.stringify(normalizeSnapshot(snapshot));

  await db.prepare(`
    INSERT INTO settings_transfers (
      game, email, activation_code, snapshot_json, source_device_id, created_at, updated_at
    )
    VALUES (?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(game, email, activation_code)
    DO UPDATE SET
  ...
  return await getExistingTransfer(db, game, email, activationCode);
}
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const activationCode = normalizeCode(url.searchParams.get("activation_code"));
    ...
    const row = await getExistingTransfer(db, game, email, activationCode);
    if (!row) {
      return json(request, {
        ok: true,
        exists: false,
    ...
    return json(request, {
      ...
      game,
      email,
      sourceDeviceId: String(row.source_device_id || snapshot.sourceDeviceId || "").trim(),
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (snapshotSize > 250_000) {
      return bad(request, "SNAPSHOT_TOO_LARGE", 413);
    }

    const verified = await verifyOwnership(db, request, game, email, activationCode);
    if (!verified.ok) return bad(request, verified.error, verified.status || 400);

    const row = await saveTransfer(db, game, email, activationCode, snapshot);
